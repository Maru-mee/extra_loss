...
* 解像度512px未満の画像ではいくつかのloss計算がskipされます。低解像度のノイズを拾わないようにするため。
  ...
  * この設計があるために、SD1では効果をほぼ得られません（モデルが512pxを超過する画像を学習できない性質であるため）。それでも使用したい場合は手動でコードを書き換えてください。area_lower_limit_imgの値を引き下げ、かつ、細かい分割を行ういくつかのlossの機能をOFFにするなど。
* ε-pred, v-predに対応。
...
* learning rateは既存設定を流用可能ですが、少し下げる必要があるかもしれません。これまで認知できなかったgradが発生するためです。
* optimizerのbeta値は既存設定を流用可能です。なお、より低いbeta値でも破綻しにくくなっています（従来はピクセルごとの一致しか考慮されていないため、ノイズまみれになりやすい傾向であったため）
